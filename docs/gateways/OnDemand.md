...
| Input | Description |
| --- | ----------- |
| Cluster | Possible options are [Alpine](docs/clusters/Riviera/index.md) and [Blanca](../clusters/blanca/blanca.md).  |
...
| Partition | Specifies a particular node type to use. For example, you can provide `ahub` for quicker access or utilize another [partition on Alpine](riviera-hardware.md#partitions).  Blanca users may use their `blanca-<groupname>` partition.  |
...
| Memory [GB]<sup>1</sup> | The total amount of memory allocated for the Job. Memory in GB should be roughly the number of cores multiplied by 3.8 for standard Alpine `amilan` nodes. For specifications on memory for each Alpine partition, see [Alpine hardware](riviera-hardware.md). |
| QoS Name | Quality of Service (QoS) constrains or modifies certain job characteristics. On most Alpine partitions you can specify `normal` for jobs of up to 24 hours and `long` for jobs of up to 7 days in duration. For more information see [Alpine QoS](riviera-hardware.md#quality-of-service-qos). Blanca users should specify their `blanca-<groupname>` for QoS. |
