# Riviera: condo FairShare and resource access 

This purpose of this document is to provide a detailed overview of 
Riviera's FairShare and resource access policies. The intended audience is 
Riviera's insitutional-level condo contributors.

Updated November 4, 2022 

## Goals and philosophy 

The Riviera supercomputer hosted at CSU and administered by Research 
Computing at CSU (DSRI) supports institutional condo purchases, the 
...
DSRI’s own data-gathering and monitoring, and other factors. 

## Node, partition, and FairShare policies 

### Overview 

All nodes, whether contributed by CSU or by another institution, 
will be added to a shared partition to which all Riviera users will have 
access. This does not preclude adding nodes to one or more additional 
partitions with different settings and attributes. At minimum, DSRI will 
create one dedicated partition and QOS per contributor. 

By configuring all nodes into a common pool by default, DSRI intends to 
better maximize system utilization and ensure that users are not locked 
out of Riviera due to (for example) power or networking issues that only 
...
As one of several determinants of job priority, DSRI will assign a 
FairShare score in the Slurm database for each condo buy-in on Riviera 
...
To compute the GPU term in the FairShare equation, DSRI will multiply 
total contributed GPUs by an "acceleration factor". This factor is derived 
from the MATLAB GPU benchmark series, which DSRI has used to measure 
...
Riviera node, the number of SUs allocated for a 64-core node with 3 GPUs 
...
* average clock speed / minimum clock speed on Riviera) 
...
subaccount with 80%. The general/projects pattern is already in place on Riviera. Contributors wishing for a different accounting arrangement from "general/projects" should reach out to DSRI with specific requests. This crediting and billing system is intended to balance contributions and 
utilization. 

### Example and comment on FairShare and billing 

Suppose the lowest rated clock speed available on any Riviera node is 
...
Additionally, DSRI reserves the authority to modify this policy or 
...
DSRI will provide regular reports to institutional Riviera partners to 
...
DSRI will generate automated reports monthly and share them through 
appropriate channels, either written or in-person. DSRI will also produce 
...
can apply these changes quickly once DSRI and the contributor(s) agree 
upon them. 
...
to the "Multifactor Priority Plugin") or follow up with DSRI with specific 
questions. This policy is now in effect and is subject to change at any 
time. 


Riviera is jointly funded by the University of Colorado Boulder, the 
